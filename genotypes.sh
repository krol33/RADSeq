...
python $script_dir/retreat_haplotypes.tsv.py -i $input/batch_1.haplotypes_1.tsv -s $input/input -o $input/batch_1.haplotypes_amb.tsv
...
python $script_dir/genetics_stat.py -d -g -i $input/locus_polymorphes_cov.txt -p $map -o $STAT_DIR -n $input/all_locus_polymorphes
...
awk -v TOT=$NB_IND '{if($1!="Catalog"){tab[$2]++}}END{for (i=1; i<= TOT; i++){print i"\t"tab[i]}}' $input/batch_1.haplotypes_amb_ord.tsv > $STAT_DIR/nb_indiv_genotyped_per_locus.txt 
awk -v TOT=$NB_IND '{if($1!="Catalog"){tab[$2]++}}END{for (i=1; i<= TOT; i++){print i"\t"tab[i]}}' $input/locus_polymorphes_cov.txt > $STAT_DIR/nb_indiv_genotyped_per_locus_polymorphes.txt
echo -e "\nsee detailed number of cluster in function of number of genotyped individuals available: " $input/nb_indiv_genotyped_per_locus.txt", "$input/nb_indiv_genotyped_per_locus_polymorphes.txt" and "$input/nb_indiv_genotyped_per_locus.jpg  >> $STAT_DIR/summary_statistics.txt
