...
# -t = -d -r dans ustacks
cmd_line="perl /usr/local/bioinfo/src/Stacks/stacks-1.35/scripts/denovo_map.pl $sample -o $OUT_DIR -t -m $MIN_DEPTH -M $MAX_PRIM_DIST -N $MAX_SEC_DIST -n $MISMATCH -T 10 -O $POP_FILE -b 1 -i 1 -e $stacks_dir -S "
if [[ $MAC_LOCUS_STACKS != "" ]]
then
